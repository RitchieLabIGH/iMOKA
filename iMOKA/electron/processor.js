		  ...
		  if (this.matrices && ! update){
			  ...
			  if ( this.isInit()){
				  ...
				  if (this.options.connection_type == 'local'){
						  ...
						  files.forEach(s_dir =>{
							  if ( s_dir.isDirectory() ){
								  if (fs.existsSync(matrix_dir+"/"+s_dir.name+"/matrix.json")){
									...
									// TODO: Check that sample of the matrix exists, otherwise give an error message
					  ...
					  this.getSSH().then((ssh)=>{
						  ssh.execCommand("mkdir -p "+matrix_dir+" && ls -l "+matrix_dir +" | awk '/^d/ {print $NF}'").then((res)=>{
							  this.matrices=[];
							  if (res.code == 0 && res.stdout.length > 1){
								  let muids= res.stdout.split("\n"), promises=[];
								  muids.forEach((muid)=>{
									  promises.push(new Promise((resolve, reject)=>{
										  ssh.execCommand("cat "+matrix_dir+"/"+muid+"/matrix.json").then((res)=>{
											  if (res.code == 0 && res.stdout.length > 0){
												  let curr_matrix = JSON.parse(res.stdout);
												  ssh.execCommand("cat "+matrix_dir+"/"+muid+"/reduced.matrix.json").then((red_res)=>{
													  if (red_res.code == 0 && red_res.stdout.length > 0){
														  curr_matrix.reduced = JSON.parse(red_res.stdout)
													  }
													  ssh.execCommand("cat "+matrix_dir+"/"+muid+"/aggregated.info.json").then((aggr_res)=>{
														  if ( aggr_res.code == 0 && aggr_res.stdout.length > 0 ){
															  curr_matrix.aggregated = JSON.parse(aggr_res.stdout);
														  }
