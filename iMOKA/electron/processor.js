		...
		"completed_job_parse": (stdout) => {
			...
			if (tmp.length >= 3) {
				...
				for (let i = 2; i < tmp.length; i += 3) {
					...
					out_i.times = {
						completed: Date.parse(out_i.raw.End),
...
class Processor {
	...
	deleteSamples(samples_names) {
		return new Promise((resolve, reject) => {
			if (this.isInit()) {
				let errors = "";
				console.log(samples_names)
				for (let sam in samples_names) {
					let sam_dir= this.options.storage_folder + "/samples/" + samples_names[sam]+"/"
					if (this.options.connection_type == 'local') {
						let err=rimraf.sync(sam_dir);
					} else {
						this.getSSH().then((ssh) => {
							ssh.execCommand("ls " + sam_dir + " && rm -fr " + sam_dir).then((res) => {
				...
				if ( errors.length > 1 ){
					...
					resolve("Deleted all the samples.")
				}
	...
	getRemoteFolder(remote_path, local_path) {
		return new Promise((resolve, reject) => {
			...
			this.getSSH().then((ssh) => {
				ssh.getDirectory(local_path, remote_path, {
					recursive: true, tick: (pA, pB, error) => {
						if (error) {
							processing.err += 1
						} else {
							processing.ok += 1
	...
	importKmerList(request) {
		return new Promise((resolve, reject) => {
			...
			let new_matrix = {
				k_len: agg.k_len, name: request.new_name, imported: true, uid: agg.uid, groups: agg.groups,
			...
			try {
				...
				if (!fs.existsSync(this.options.storage_folder + "/.singularity/iMOKA") || this.options.update) {
					if (this.options.remote_image) {
						...
						download(win, this.options.original_image, {
							directory: this.options.storage_folder + "/.singularity/", filename: "iMOKA", onProgress: (prg) => {
