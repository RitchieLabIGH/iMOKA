...
class iMokaBE extends EventEmitter {
		...
		return new Promise((resolve, reject) => {
			if (this.processor) {
				this.processor.getSamples(request).then(samples => {
					resolve({
						"message": "SUCCESS", code: 0, data: samples,
				...
				resolve({
					"message": "Processor not loaded", code: 1, data: [],
	...
	getMatrices(request) {
		return new Promise((resolve, reject) => {
			this.updateMatrices().finally(() => {
				resolve({
					"message": "SUCCESS", code: 0, data: this.user_session.data.matrices, draw: request.draw,
	...
	getIdeogram(request) {
		return new Promise((resolve, reject) => {
			...
			let annotations = {
				keys: ["name", "start", "length", "class", "expression"],
				annots: []
			}, aln, hexpr;
	...
	getKmers(event, id, request) {
		return new Promise((resolve, reject) => {
			...
			this.user_session.save();
			resolve({
				"data": data_to_send, "message": "SUCCESS", "draw": request.draw, code: 0,
	...
	removeProfile(request) {
		return new Observable((observer) => {
			if (request.profile_number < this.user_session.data.profile.process_config.profiles.length) {
				this.user_session.data.profile.process_config.profiles.splice(request.profile_number, 1);
