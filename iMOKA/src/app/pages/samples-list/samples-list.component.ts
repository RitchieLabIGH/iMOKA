...
export class SamplesListComponent implements OnInit {
	...
	refreshTable(event?: any, update?: boolean) {
		if (event) {
			...
			if (event == "reset") {
				this.dtOptions.search.value="";
				this.dtOptions.metadataKeyFilter= [];
				this.dtOptions.metadataKeyValFilter= {};
			}
		}
		if (update) {
	...
	deleteSamples() {
		...
		if (samples.length == 0) {
			this.snackMessage({ title: "No samples selected", message: "Select one or more samples to delete", opts: {} });
			return;
		} else {
			this.sampleService.deleteSamples(samples).then((res) => {
				this.snackMessage({ title: "Done!", message: "Deleted " + samples.length + " sample" + (samples.length > 1 ? "s." : ".") });
			}).catch((err) => {
				this.snackMessage({ title: "ERROR", message: err.message ? err.message : err });
			})
		}
