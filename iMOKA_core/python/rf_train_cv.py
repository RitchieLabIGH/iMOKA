...
def run(training, cross_v, perc_test, file_output, n_trees=100, nmodels=1, max_features = 20, cpus=-1 ):
    ...
    for i in range(0, nmodels):
        ...
        for j in range(2, max_features+1):
            fsel.max_features=j
            clf=clone(base_clf)
            tmp_acc =  cross_validate(clf, fsel.transform(values), Y, scoring="balanced_accuracy", cv=ShuffleSplit(n_splits=cross_v, test_size=perc_test), n_jobs=cpus,  return_train_score=True)
...
def main():
    parser = argparse.ArgumentParser(description="Assert the accuracy training set.");
    ...
    parser.add_argument("-c", "--cross-validation", default=5, type=int, help="The number of fold cross validation to assert the accuracy of the models. Default: 5")
    parser.add_argument("-p", "--percentage-test", default=0.25, type=float, help="The percentage of samples to consider during the crossvalidation. Default: 0.25")
    ...
    run(args.train,args.cross_validation,args.percentage_test, args.output,args.n_trees,  args.rounds , args.max_features, args.threads);
    return 
