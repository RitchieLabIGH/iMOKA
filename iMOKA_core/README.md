...
``` 
singularity dump TODO iMOKA.img
```
...
The preprocessing step converts takes as input biological reads and count the k-mers using KMC3 (( TODO: REF kmc3). 
...
The refined lists can be used to build classification models, for example using the Random Forest model generator included in iMOKA, a more explorative algorithm as GECKO (TODO link) or a custom software. 
Furthermore iMOKA offers a self organizing map based clusterization module that allows to clusterize the k-mers in order to have a visual rapresentation of the samples. (TODO improve this description)
...
The extended version of iMOKA contains a prebuilt version of the human reference hg38 gmap/gsnap database. You can use it with the default argument. 
```
...
The [aggregation step](#fr2) can use a mapper to increase the efficiency of the redundancy reduction and to have a first level of annotation. The singularity image contains gmap and STAR. 
...
```
The extended version of iMOKA (6.4 Gb) uses gmap and a pre-installed version of gmap/gsnap database of the human reference genome hg38 ( GENCODE v29 ), embedded in the singularity image in the position /gmap_ref/. 

To print the default configuration JSON file, you can use the command:
```
singularity run iMOKA.img iMOKA_core config
```
If you prefer a lighter version, you can use the basic image (900Mb).
