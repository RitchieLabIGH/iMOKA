...
void Gene::init(std::string geneId, std::string geneName, double coverage_limit,
		...
		if (cmax > max_val) {
			if (perfect_match) {
				for (auto i : alignmentDerivedFeatures) {
					...
					if (sa.best_kmer == all_sequences[mrl.query_index].best_kmer
							&& sa.signature_type != "splice") {
						cmax = 0;
					}
				}
			}
			...
			if (intron_cov > coverage_limit) {
				...
				for (int64_t e = events.size() - 1; e > 0; --e) {
					if (events[e].gene_name.count(gene_name) != 0
						...
						found = true;
						if (events[e].type == "intron") {
							events[e].info += "\n" + transcript.first
									+ " intron " + std::to_string(i) + " cov "
	...
	for (auto i : alignmentDerivedFeatures) {
		const AlignmentDerivedFeature &sa = all_signatures[i];
		if (ads_stats.count(sa.signature_type) == 0)
			ads_stats[sa.signature_type] = 0;
...
json Gene::to_json() {
	...
	json out = { { "gene_id", gene_id }, { "name", gene_name }, { "chromosome",
			chromosome }, { "ads", ads_stats }, { "id", id }, { "alignments",
			alignments }, { "best_kmer", best_kmer->id }, { "coverages",
			coverage } };
