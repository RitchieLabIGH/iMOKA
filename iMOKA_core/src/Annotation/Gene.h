...
public :
	Gene(){};
	Gene(std::string gene_name, std::string gene_id, double coverage_limit,std::map<std::string, std::vector<Segment>> transcript_exons,  std::set<uint64_t> mapper_results_idxs,  std::vector<MapperResultLine> & mapper_results,  std::vector<AlignmentDerivedFeature> & all_signatures , std::vector<GraphSequence> & all_sequences, bool perfect_match){
