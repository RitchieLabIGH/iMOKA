...
bool Classification::run(int argc, char** argv) {
	try {
		if (std::string(argv[1]) == "reduce") {
			cxxopts::Options options("iMOKA reduce", "Reduce a k-mer matrix in according to the classification power of each k-mer");
			options.add_options()("i,input", "The input matrix JSON header",
					...
					("s,standard-deviation", "Standard deviation to achieve convergence in cross validation. Suggested between 0.5 and 2",
							cxxopts::value<double>()->default_value("0.5")	)
					("b,batch",
					"format: \"a-b\" -> Process only the baches from a to b.",
					cxxopts::value<std::string>()->default_value("all"));
			auto parsedArgs = options.parse(argc, argv);
			if (parsedArgs.count("help") != 0|| IOTools::checkArguments(parsedArgs, {"input", "output"}, log)) {
		...
		if (std::string(argv[1]) == "cluster") {
			cxxopts::Options options("iMOKA cluster", "Reduce a k-mer matrix in according to the clusterization power of each k-mer");
				options.add_options()
						("i,input", "The input matrix JSON header", cxxopts::value<std::string>())
						("o,output", "Output matrix file", cxxopts::value<std::string>())
						("h,help", "Show this help")
						("m,method", "Clusterization method to use [DBSCAN|KMEANS]. Use DBSCAN if you don't know the number of clusters that are present in your data, use KMEANS if you know it.",cxxopts::value<std::string>()->default_value("DBSCAN"))
						("c,clusters", "min-max:dim number of clusters and minimum dimension of a cluster.", cxxopts::value<std::string>()->default_value("2-10:2"))
						("e,epsilon", "Epsilon value in DBSCAN. Two points are considered neighbors if the distance between the two points is below the threshold epsilon. The points are rescaled in [0-100]", cxxopts::value<double>()->default_value("5"))
						("s,silhuette", "Minimum median silhuette required to consider a k-mer", cxxopts::value<double>()->default_value("0.90")	)
						("b,batch", "format: \"a-b\" -> Process only the baches from a to b.", cxxopts::value<std::string>()->default_value("all"));
			auto parsedArgs = options.parse(argc, argv);
			if (parsedArgs.count("help") != 0 || IOTools::checkArguments(parsedArgs, {"input", "output"}, log)) {
					log << "Help for the classification \n" << options.help() << "\n";
					return false;
				}
			return clusterizationFilter(
		...
		if (std::string(argv[1]) == "models") {
			cxxopts::Options options("iMOKA models", "Use the Genetic algorithm to find the combinations of features able to classify your samples.");
			options.add_options()("i,input",
					...
					("s,standard-deviation", "Standard deviation to achieve convergence in cross validation",
												cxxopts::value<double>()->default_value("0.05")	)
					(
...
#pragma omp parallel firstprivate(cross_validation, sd, min_acc, perc_test, adjustments )
{
	std::function<std::vector<double>(const std::vector<std::vector<double>> &,
							...
							const std::map<uint64_t, uint64_t>, const uint64_t, const double, double)> fun  = MLpack::pairwiseNaiveBayesClassifier;
	...
	double minEntropy = 0, localMinEntropy=max_entropy, adj_down=adjustments[0], adj_up=adjustments[1];
	uint64_t tot_lines = 0, kept=0, entropy_update_every=30, last_update=0;
	uint64_t a = thr * batch_size, b = ((thr + 1) * batch_size) - 1;
	Kmer from_kmer(k_len, a), to_kmer(k_len, b);
	mat.go_to(from_kmer);
	std::ostringstream os;
	os << "[ " << IOTools::now() << " ] Thread " << thr << " starting on cpu " << sched_getcpu() << ", memory occupied: " << IOTools::format_space_human(IOTools::getCurrentProcessMemory())  << ".\n";
		...
		if ( line.getKmer() <= to_kmer ){
			...
			if (entropy >= minEntropy) {
				...
				for (double & v : res)
					keep = keep || v >= min_acc;
				if (keep) {
					localMinEntropy = localMinEntropy < entropy ? localMinEntropy: entropy;
					last_update++;
					kept++;
					if ( last_update >=  entropy_update_every ){
						minEntropy = minEntropy == 0 ?
											localMinEntropy - (localMinEntropy * adj_down * 2) :
											localMinEntropy - (localMinEntropy * adj_down ) < minEntropy ?
												minEntropy - (localMinEntropy * adj_down) :
			...
			if ( tot_lines % 1000000  == 0 ){
				...
				tlog << perc << "\t" << tot_lines << "\t" << kept << "\t" << minEntropy << "\t" <<IOTools::format_time(std::chrono::duration_cast<std::chrono::seconds>(
	...
	ofs.close();
	std::cerr << "[ " << IOTools::now() << " ] Thread " << thr << " completed. " << tot_lines << " k-mers analysed, "<< kept << " kept \n";
	tlog << "100\t" << tot_lines << "\t" << kept << "\t" << minEntropy << "\t" <<IOTools::format_time(std::chrono::duration_cast<std::chrono::seconds>(
	std::chrono::high_resolution_clock::now() -start).count())<< "\n";
	tlog.flush();
	tlog.close();
} // parallel end
...
/// @param clusters
/// @param min_cluster
bool Classification::clusterizationFilter(std::string input, std::string output, std::string method,
		std::string batch, double silhuette, double epsilon, std::string clusters){
	BinaryMatrix mat(input, true);
	std::ofstream ofs(output);
	size_t min_cluster , max_cluster, min_dim, num_samples=mat.col_names.size(), number_of_dimensions=100;

	try {
		min_cluster=std::stoi(clusters.substr(0, clusters.find(":")).substr(0, clusters.find("-")));
		max_cluster= std::stoi(clusters.substr(0, clusters.find(":")).substr(clusters.find("-")+1));
		min_dim =std::stoi(clusters.substr(clusters.find(":")+1));
		std::cerr << "min cluster: " << min_cluster << "\nmax_cluster: " << max_cluster << "\nmin_dim: " << min_dim << "\n";
	} catch ( std::exception & e){
		log << "ERROR! cluster size range have to be given as min-max:dim ( example: 2-4:2 ).\n";
		log << clusters <<"\n";
		return false;
	}

	std::function<ClusterizationResult(const std::vector<std::vector<double>> &)> fun;
	json info = { { "silhuette", silhuette }, { "method", method }, { "cluster_range", clusters },  { "file_in", input }, { "file_out", output } };
	if (method == "DBSCAN") {
	...
	log << "Batch\tTotLines\tPerc\tReadingTime\tProcessTime\tTimeSinceStart\tExpectedRemainingTime\tMemoryUsage\n";
	log.flush();
	...
	std::string expected_end, reading_time, total_time, process_time;
	arma::Mat<double> distance = arma::zeros<arma::Mat<double>>( num_samples,num_samples );
	...
	while (running) {
		...
		if (current_batch >= from_batch ) {
			...
			for ( int t=0; t < omp_get_max_threads(); t++){
				distances.push_back(arma::zeros<arma::Mat<double>>( num_samples,num_samples));
			}
			log << current_batch << "\t"; log.flush();
#pragma omp parallel
			{
				std::ostringstream final("");
				ClusterizationResult res= fun(  buffer[omp_get_thread_num()] );
				log << omp_get_thread_num() << " " << res.number_of_clusters << " "  << res.silhuettes_score << "\n"; log.flush() ;
				if ( res.silhuettes_score > silhuette && res.number_of_clusters >= min_cluster && res.number_of_clusters <= max_cluster ) {
					final << res.silhuettes_score  << "\t" << res.number_of_clusters << "\t";
			...
			log << tot_lines<< "\t"<< mat.perc() << "\t" << reading_time << "\t" <<process_time << "\t"
					<< total_time << "\t" << expected_end << "\t" <<IOTools::format_space_human(IOTools::getCurrentProcessMemory())  << "\n";
			log.flush();
			b_start = std::chrono::high_resolution_clock::now();
			uint64_t success=0;
			...
			if (success > 0 ){
				...
				for ( auto n : mat.col_groups) matrix_file << n << "\t";
				matrix_file << "\n";
				for ( auto n : mat.col_names) matrix_file << n << "\t";
				matrix_file << "\n" << distance << "\n";
				matrix_file.close();
			}
			ofs.flush();
...
bool Classification::geneticAlgorithm(std::string list_of_kmers,
		...
		uint64_t cross_validation, double sd, double perc_test, uint64_t max_dimension,
		uint64_t population, uint64_t generation_max, double CO_fraction,
		double MUT_rate, double GENE_MUT_rate, uint64_t ELITE_count,
		uint64_t total_runs, uint64_t numTrees, uint64_t minLeaf,
		uint64_t best_stall_max, uint64_t average_stall_max) {
	...
	static std::function<
			double(const std::vector<std::vector<double>>,
					const std::vector<uint64_t>,
					const std::map<uint64_t, uint64_t>, const uint64_t, const double, double)> objective;
	static std::function<
			json(const std::vector<std::vector<double>>,
					const std::vector<uint64_t>,
					const std::map<uint64_t, uint64_t>, const uint64_t, const double, double)> best_model;
	if (method == "SM" || method == "LR") {
		...
		objective =
				[numTrees, minLeaf] (const std::vector<std::vector<double>> values,
					...
					return MLpack::randomForestClassifier(values, groups, group_counts, crossValidation,sd,  perc_test, numTrees, minLeaf);
				};
		best_model =
				[numTrees, minLeaf](const std::vector<std::vector<double>> values,
						const std::vector<uint64_t> groups,
						const std::map<uint64_t, uint64_t> group_counts,
						const uint64_t crossValidation,const double sd ,  double perc_test ) {
					return MLpack::randomForestClassifierModelBagging(values, groups, group_counts, crossValidation, sd,  perc_test, numTrees, minLeaf);
				};
	...
	for (int run = 0; run < total_runs; run++) {
						...
						const MySolution& best_genes)
				{
					...
					if ( ga_obj.verbose ) {
						...
						for (int i=0; i < best_genes.size(); i++) {
							std::cout << (i==0? "" : ",") << (best_genes[i] == n_features ? "None" : rows[best_genes[i]]->getName() );
						}
						std::cout <<")"<<", " <<"Exe_time="<<last_generation.exe_time <<std::endl;
					}
				};
		ga_obj.init_genes =
				[&]( MySolution & p, const std::function<double(void)> &rnd01 ) {
					...
					for (int i=0; i < max_dimension; i++ ) {
						...
						if ( rnd_num > (1-GENE_MUT_rate) ) {
							...
							if ( std::find(p.begin(), p.end(), ordered_genes[new_num]) == p.end()) { // Check that the gene is not already in the individual
		...
		ga_obj.eval_solution =
				[&](const MySolution & p, MyMiddleCost & c) {
					...
					for ( auto i : p) if ( i!= n_features ) {
						data.push_back(rows[i]->count);
					}
					if (data.size() > 0) {
						c=objective( data, groups, group_counts, cross_validation,sd, perc_test);
						c=100-c;
						return true;
					} else {
						c=100;
		...
		ga_obj.crossover = [](const MySolution& X1,
				const MySolution& X2,
		...
		json model = best_model(data, groups, group_counts, cross_validation,sd,
				perc_test);
