...
public:
	...
	bool classificationFilterMulti(std::string file_in, std::string file_out, int min , uint64_t entropy_evaluation , uint64_t cv,double sd, double acc, double perc_training, double confidence, std::vector<double> adjustments,  bool limit_counts, bool logarithmic);
	...
	bool clusterizationFilter(std::string file_in, std::string file_out, uint64_t nbins, double sigthr, bool log);
};
}
} /* namespace kma */
